...
public class CollisionChecker {
	...
	public void checkTile(Entity entity) {
		...
		try {
			...
			case "left":
				...
				if (tileNum1 > 9) {
					String sTile = String.valueOf(tileNum1);
					int firstTile = Integer.valueOf(sTile.substring(0, 1));
					int secondTile = Integer.valueOf(sTile.substring(1));

					if (gp.tm.tile[firstTile].collision || gp.tm.tile[secondTile].collision) {
						tile1collision2 = true;
					}
				} else if (tileNum1 <= 9) {
					if (gp.tm.tile[tileNum1].collision) {
						tile1collision2 = true;
					}
					if (gp.tm.tile[tileNum1].waterCollision) {
						entity.waterCollision = true;
					}
				}

				if (tileNum2 > 9) {
					String sTile = String.valueOf(tileNum2);
					int firstTile = Integer.valueOf(sTile.substring(0, 1));
					int secondTile = Integer.valueOf(sTile.substring(1));
					...
					if (gp.tm.tile[tileNum2].collision) {
						tile2collision2 = true;
					}
					if (gp.tm.tile[tileNum2].waterCollision) {
						entity.waterCollision = true;
					}
				}
	...
	public void checkHostile(Hostile h) {
		int ex = h.worldX;
		int ey = h.worldY;
		...
		if (dist <= (gp.tileSize * 5)) {
			h.aggro = true;
		}
		if (dist <= (gp.tileSize * 0.5)) {
			gp.player.health -= 5;
